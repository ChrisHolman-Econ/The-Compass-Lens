...
* **Relative Velocity:** Washtenaw County's labor force growth has outpaced the other counties up until Septemer 2025 when it experienced a downturn. Estimates in May 2026 shows the labor force has increased marginally and resumes tracking the progress in neighboring counties in the OWL Corridor.
...
> **Data Note:** *Metrics reflect official LAUS/BLS benchmarks as of `r format(Sys.Date(), "%B %d, %Y")`. Historical series incorporate all revised estimates released through `r latest_date_str`. In October 2025, a federal government shutdown which prohibited the collection of LAUS data. There is no LAUS data for October 2025.*
